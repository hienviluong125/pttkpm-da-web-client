...
export default {
  ...
  mounted() {
    ...
    axios
      ...
      .then((res) => {
        const { success, workspace } = res.data;
        if (success) {
          this.workspace = workspace;
          this.workspace.workspace_services = this.workspace.WorkspaceServices.map(
            (ws) => ({ name: ws.name })
          );
          this.workspace.attachments = this.workspace.Attachments.map(
            (atta) => ({ url: atta.url })
          );
        }
        // console.log(res);
  ...
  methods: {
    ...
    onSubmit(e) {
      e.preventDefault();
      axios
        .post(`/api/workspace/${this.workspace.id}/update`, {
          ...this.workspace,
        })
        .then((res) => {
          if (res.data.success) {
            this.$router.push({ path: '/partner/workspace', query: { success: true } })
          }
        })
        .catch((err) => {
          console.log(err);
